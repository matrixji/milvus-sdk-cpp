...
install_deps_for_ubuntu_common() {
    ...
    ${SUDO} apt-get -y install python2.7 gpg wget gcc g++ ccache make \
                       libssl-dev iwyu lcov git python3 python3-pip python3-dev
...
install_deps_for_fedora_common() {
    ...
    ${SUDO} dnf -y install cmake python3 python3-pip python3-devel
...
install_deps_for_centos_7() {
    ...
    ${SUDO} yum -y install devtoolset-7 python36 python36-pip python36-devel

    # install conan, cmake
    python3 -m pip install --user -U pip
    python3 -m pip install  --user scikit-build
    python3 -m pip install  --user 'conan>=2.0' cmake
